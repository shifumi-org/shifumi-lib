                                    ...
                                    if ($expireTime <= $end_time) {
                                        ...
                                        //if ($isSendNotification) {
                                            ...
                                            if (!in_array('on_expire', $alreadysentmsgs) && !in_array('on_expire_' . $plan_id, $alreadysentmsgs)) {
                                                ...
                                                do_action('arm_general_log_entry', 'cron', 'expired subscription email send', 'armember', 'userid='.$usr->ID.', planid='.$plan_id.', plan='.$plan_name.', msg='.$cron_msgs[$usr->ID]);
                                            ...
                                            if (!empty($expireTime)) {
                                                if ($expireTime > $now && $expireTime <= $endtime_end) {
                                                    ...
                                                    if (!in_array('before_expire_' . $message->arm_message_id, $alreadysentmsgs)) {
                                                        ...
                                                        do_action('arm_general_log_entry', 'cron', 'expire subscription check plan', 'armember', 'user_id='.$usr->ID.', plan='.$plan_id.', expireTime='.$expireTime);
