...
use uncanny_legacy_fm_plugin\Sample;
...
class QuestionAnalysisReport extends Config {
	...
	public static function analysis_report_scripts() {
		...
		wp_enqueue_script(
			'responsive-datatables-script',
			self::get_admin_js( 'dataTables.responsive', '.min.js' ),
			array(
				'jquery',
				'datatables-script',
			),
		...
		wp_enqueue_script(
			'buttons-datatables-script',
			self::get_admin_js( 'dataTables.buttons', '.min.js' ),
			array(
				'jquery',
				'datatables-script',
			),
		...
		wp_enqueue_script(
			'buttons-html-datatables-script',
			self::get_admin_js( 'buttons.html5', '.min.js' ),
			array(
				'jquery',
				'datatables-script',
				'buttons-datatables-script',
		...
		wp_enqueue_script(
			'buttons-flash-datatables-script',
			self::get_admin_js( 'buttons.flash', '.min.js' ),
			array(
				'jquery',
				'datatables-script',
				'buttons-datatables-script',
		...
		wp_enqueue_script(
			'jszip',
			self::get_admin_js( 'jszip', '.min.js' ),
			array(
				'jquery',
				'datatables-script',
				'buttons-datatables-script',
		...
		wp_enqueue_style( 'datatables-responsive-styles', self::get_admin_css( 'responsive.dataTables.min.css' ) );
		wp_enqueue_style( 'buttons-dataTables-min', self::get_admin_css( 'buttons.dataTables.min.css' ) );
	...
	private static function generate_quiz_dropdown( $quizzes ) {
		...
		$drop_down .= sprintf(
			...
			sprintf(
				__( 'Select a LearnDash %s', 'uncanny-learndash-reporting' ),
		...
		foreach ( $quizzes as $quiz_id => $quiz_name ) {
			...
			$drop_down    .= '<option value="' . $quiz_id . '" ' . $selected . '>' . $quiz_id_html . $quiz_name . '</option>';
		}
		$drop_down      .= '</select>';
	...
	private static function get_quiz_questions( $quiz_id, $pro_quiz_id ) {
		...
		$question_post_ids = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT p.ID as question_post_ID
FROM $wpdb->posts p
...
ORDER BY q.sort ASC",
	...
	private static function get_answered_correctly( $question_id ) {
		...
		$qry = apply_filters(
			...
			"SELECT COUNT(*) FROM $table WHERE `question_id` = $question_id AND `correct_count` = 1 ",
			$question_id,
			$table
		);
	...
	private static function get_times_question_asked( $question_id ) {
		...
		$qry = apply_filters(
			...
			"SELECT COUNT(*) FROM $table WHERE `question_id` = $question_id",
			$question_id,
			$table
		);
	...
	private static function get_times_this_answer_selected( $question_position, $question_id ) {
		...
		$qry = apply_filters(
			...
			"SELECT COUNT(*) FROM $table WHERE `question_id` = $question_id AND answer_data LIKE '$question_position' AND incorrect_count = 1",
			$question_id,
			$question_position,
			$table
		);
	...
	private static function get_avg_time( $question_id ) {
		...
		$qry = apply_filters(
			...
			"SELECT AVG(question_time) FROM $table WHERE `question_id` = $question_id",
			$question_id,
			$table
		);
