...
function learndash_check_coupon_is_valid( string $coupon_code, int $post_id ): array {
	...
	$course_post_type = LDLMS_Post_Types::get_post_type_slug( 'course' );
	$group_post_type  = LDLMS_Post_Types::get_post_type_slug( 'group' );
...
function learndash_increment_coupon_redemptions( int $coupon_id, int $post_id, int $user_id ): void {
	...
	$redemptions = (int) learndash_get_setting( $coupon_id, LEARNDASH_COUPON_META_KEY_REDEMPTIONS );

	learndash_update_setting( $coupon_id, LEARNDASH_COUPON_META_KEY_REDEMPTIONS, $redemptions + 1 );
}
...
/**
 * Get an attached coupon id of a course/group.
 *
 * @since 4.1.0
 *
 * @param int $post_id Course/Group ID.
...
function learndash_sync_coupon_associated_metas( int $post_id, string $field, array $ids ): void {
	if ( ! in_array( $field, array( 'courses', 'groups' ), true ) ) {
		return;
	}
...
function learndash_apply_coupon(): void {
	...
	$price = preg_replace( '/.*?(\d+(?:\.?\d+)?)/', '$1', $pricing['price'] );
	...
	wp_send_json_success(
		[
			...
			'discount'    => esc_html( learndash_get_price_formatted( $discount ) ),
			'total'       => [
				'value'        => $discounted_price,
				'stripe_value' => $stripe_price,
				'formatted'    => esc_html( learndash_get_price_formatted( $discounted_price ) ),
			],
...
function learndash_remove_coupon(): void {
	...
	wp_send_json_success(
		[
			'total'   => [
				'value'        => number_format( $pricing['price'], 2, '.', '' ),
				'stripe_value' => $stripe_price,
...
function learndash_enroll_with_zero_price(): void {
	...
	if ( learndash_is_course_post( $post ) ) {
		$transaction_data['course_id'] = $post->ID;
	} elseif ( learndash_is_group_post( $post ) ) {
		$transaction_data['group_id'] = $post->ID;
	}
	...
	if ( $transaction_id > 0 && ! ( $transaction_id instanceof WP_Error ) ) {
		/** This action is documented in includes/payments/class-learndash-stripe-connect-checkout-integration.php */
