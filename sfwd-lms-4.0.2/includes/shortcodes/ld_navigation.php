...
function learndash_navigation_shortcode( $atts = array(), $content = '', $shortcode_slug = 'ld_navigation' ) {
	...
	if ( ( ! empty( $atts['course_id'] ) ) && ( ! empty( $atts['post_id'] ) ) && ( ! empty( $atts['user_id'] ) ) ) {
		$incomplete_step_id = (int) learndash_user_progress_get_previous_incomplete_step( $atts['user_id'], $atts['course_id'], $atts['post_id'], false );
		if ( ! empty( $incomplete_step_id ) && ( $incomplete_step_id !== $atts['post_id'] ) ) {
			$can_complete = false;
		} else {
			$can_complete = true;
		}
