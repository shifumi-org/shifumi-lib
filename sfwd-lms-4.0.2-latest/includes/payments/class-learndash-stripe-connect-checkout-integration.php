...
if ( ! class_exists( 'LearnDash_Stripe_Connect_Checkout_Integration' ) ) {
	/**
	 * Stripe Connect checkout integration class.
	 */
	class LearnDash_Stripe_Connect_Checkout_Integration {
		/**
		 * Plugin options.
		 *
		 * @var array
		 ...
		 * @var string
		 */
		protected string $stripe_customer_id_meta_key;
		...
		protected function configure() {
			...
			if ( ! empty( $this->secret_key ) ) {
				...
				$this->stripe = new \Stripe\StripeClient( $this->secret_key ); // @phpstan-ignore-line
			}
		...
		public function stripe_button( $default_button ) {
			global $ld_stripe_connect_script_loaded;

			if ( ! isset( $ld_stripe_connect_script_loaded ) ) {
				$ld_stripe_connect_script_loaded = false; // phpcs:ignore
			}
			...
			if ( ! empty( $default_button ) ) {
				 ...
				 */
				$stripe_button_text = apply_filters( 'learndash_stripe_purchase_button_text', __( 'Use a Credit Card', 'learndash' ) );
			} else {
				$ld_course_slug = learndash_get_post_type_slug( 'course' );
				$ld_group_slug  = learndash_get_post_type_slug( 'group' );

				if ( class_exists( 'LearnDash_Custom_Label' ) ) {
					if ( $ld_course_slug === $this->course->post_type ) {
					...
					} elseif ( $ld_group_slug === $this->course->post_type ) {
						/** This filter is documented in includes/payments/class-learndash-stripe-connect-checkout-integration.php */
					...
					if ( $ld_course_slug === $this->course->post_type ) {
						/** This filter is documented in includes/payments/class-learndash-stripe-connect-checkout-integration.php */
						$stripe_button_text = apply_filters( 'learndash_stripe_purchase_button_text', __( 'Take This Course', 'learndash' ) );
					} elseif ( $ld_group_slug === $this->course->post_type ) {
						/** This filter is documented in includes/payments/class-learndash-stripe-connect-checkout-integration.php */
						$stripe_button_text = apply_filters( 'learndash_stripe_purchase_button_text', __( 'Enroll in Group', 'learndash' ) );
					}
				}
			}
			...
			$stripe_button_nonce = wp_create_nonce( 'stripe-connect-nonce-' . $course_id . $course_price . $course_price_type );
			...
			<?php
			if ( ! $ld_stripe_connect_script_loaded ) {
				$this->button_scripts();
				$ld_stripe_connect_script_loaded = true; // phpcs:ignore
			}
		...
		public function button_scripts() {
			...
			?>
			<script src="https://cdn.jsdelivr.net/npm/js-cookie@3.0.0-rc.1/dist/js.cookie.min.js"></script> <?php //phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript ?>
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function ($) {
					...
					var ld_stripe_connect_ajaxurl = <?php echo wp_json_encode( admin_url( 'admin-ajax.php' ) ); ?>;
		...
		public function process_webhook(): void {
			if ( ! isset( $_GET['learndash-integration'] ) || $_GET['learndash-integration'] != 'stripe-connect' ) { // phpcs:ignore
				return;
			}
			...
			/**
			 * Filters whether to process Stripe webhook or not.
			 *
			 * True by default.
			 *
			 * @since 4.0.0
					...
					if ( $plan_id ) {
						...
						if ( ! empty( $course_id ) ) {
							...
							if ( ! empty( $session->subscription ) && ! empty( $course_recurring_times ) ) {
								...
								$subscription = \Stripe\Subscription::retrieve( $session->subscription ); // @phpstan-ignore-line
		...
		/**
		 * Associates course with a user.
		 *
		 * @param int|null $course_id Post ID of a course.
		 * @param int      $user_id ID of a user.
		 */
		protected function add_course_access( ?int $course_id, int $user_id ) {
			$this->update_course_access( $course_id, $user_id );
		}
		 ...
		 * @param int $user_id User ID.
		 */
		protected function remove_course_access( int $course_id, int $user_id ): void {
			$this->update_course_access( $course_id, $user_id, true );
		}
		 ...
		 */
		protected function update_course_access( int $course_id, int $user_id, bool $remove = false ): void {
			...
			$post_type_slug = get_post_type( $course_id );

			if ( learndash_get_post_type_slug( 'course' ) === $post_type_slug ) { // @phpstan-ignore-line
				ld_update_course_access( $user_id, $course_id, $remove ); // @phpstan-ignore-line
			} elseif ( learndash_get_post_type_slug( 'group' ) === $post_type_slug ) { // @phpstan-ignore-line
				ld_update_group_access( $user_id, $course_id, $remove ); // @phpstan-ignore-line
			}
		}
		 ...
		 */
		public function record_transaction( object $session, int $course_id, int $user_id, string $user_email ) {
			...
			$transaction = array(
				...
				'stripe_price_type'     => 'payment' === $session->mode ? 'paynow' : 'subscribe',
			...
			$post_id = wp_insert_post(
				array(
					'post_title'  => "Course {$transaction['stripe_name']} Purchased By {$user_email}",
		...
		protected function get_payment_methods(): array {
			$enabled_payment_methods = $this->options['payment_methods'] ?? array( 'card' );
			/**
		...
		public function ajax_init_checkout() {
			$action = 'stripe-connect-nonce-' . $_POST['stripe_course_id'] . $_POST['stripe_price'] . $_POST['stripe_price_type']; // phpcs:ignore
		...
		/**
		 * Hides the plugin's Stripe button to avoid 2 Stripe buttons.
		 */
		private function hide_plugin_stripe_button(): void {
			...
			if ( ! is_null( $ld_stripe_checkout ) ) {
				remove_filter( 'learndash_payment_button', array( $ld_stripe_checkout, 'payment_button' ), 10 );
			} else {
				global $ld_stripe_legacy_checkout;
				if ( ! is_null( $ld_stripe_legacy_checkout ) ) {
					remove_filter( 'learndash_payment_button', array( $ld_stripe_legacy_checkout, 'payment_button' ), 10 );
				}
			}
		}
	}

	global $ld_stripe_connect_checkout;
	$ld_stripe_connect_checkout = new LearnDash_Stripe_Connect_Checkout_Integration(); // phpcs:ignore
}
