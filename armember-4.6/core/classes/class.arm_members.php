    ...
    class ARM_members {
        ...
        function arm_clear_debug_logs_data()
        {
            if(!empty($_POST) && !empty($_POST['arm_clear_debug_log_item']))
            {
                ...
                if($arm_clear_debug_log_item=='optins' || $arm_clear_debug_log_item=='cron')
                {
                                        ...
                                        foreach ($users as $key => $u) {
                                            ...
                                            if (is_user_member_of_blog($user_id)) {
                                                ...
                                                $users_data[$user_id] = array(
                                                    'id' => htmlspecialchars(utf8_encode($user_id), ENT_NOQUOTES),
                                                    'username' => htmlspecialchars(utf8_encode($user_info->user_login), ENT_NOQUOTES),
                                                    'email' => htmlspecialchars(utf8_encode($user_info->user_email), ENT_NOQUOTES),
                                                    'status' => htmlspecialchars(utf8_encode($statusText), ENT_NOQUOTES),
                                                    'role' => htmlspecialchars(utf8_encode($roles), ENT_NOQUOTES),
                                                    'subscription_plan' => htmlspecialchars(utf8_encode(implode(",", $arm_user_plan)), ENT_NOQUOTES),
                                                    'joined' => htmlspecialchars(utf8_encode($user_info->user_registered), ENT_NOQUOTES),
                                                );
                                                if(!$is_multiple_membership_feature->isMultipleMembershipFeature)
                                                {
                                                ...
                                                if (isset($request['arm_user_metas_to_export']) && $request['arm_user_metas_to_export'] != '') {
                                                    ...
                                                    if (in_array('first_name', $user_meta)) {
                                                        $users_data[$user_id]['first_name'] = htmlspecialchars(utf8_encode($user_info->first_name), ENT_NOQUOTES);
                                                    }
                                                    if (in_array('last_name', $user_meta)) {
                                                        $users_data[$user_id]['last_name'] = htmlspecialchars(utf8_encode($user_info->last_name), ENT_NOQUOTES);
                                                    }
                                                    ...
                                                    if (in_array('display_name', $user_meta)) {
                                                        $users_data[$user_id]['display_name'] = htmlspecialchars(utf8_encode($user_info->display_name), ENT_NOQUOTES);
                                                    }
