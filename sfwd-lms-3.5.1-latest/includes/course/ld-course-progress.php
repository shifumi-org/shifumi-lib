 ...
 *
 * @return WP_Post The `WP_Post` object of lesson/topic to be completed.
 */
	...
	if ( ( ! empty( $quiz_results ) ) && ( is_array( $quiz_results ) ) ) {
		foreach ( $quiz_results as $quiz ) {
			...
			if ( ( -1 === $course_id ) || ( $course_id === $quiz['course'] ) ) {
				...
				if ( ( isset( $quiz['graded'] ) ) && ( ! empty( $quiz['graded'] ) ) ) {
					$check_graded = true;
					if ( has_filter( 'learndash_quiz_complete_essay_grded' ) ) {
						/**
						 * Filter to check quiz if check graded item(s).
						 *
						 * @since 3.5.0
						 * @deprecated 3.4.0 Use {@see 'learndash_quiz_complete_essay_graded'} instead.
						 *
						 * @param bool  $check_graded  True to check quiz graded status.
						 * @param int   $user_id       User ID.
						 * @param array $quiz_progress User quiz progress.
						 */
						$check_graded = (bool) apply_filters_deprecated( 'learndash_quiz_complete_essay_grded', array( $check_graded, $user_id, $quiz ), '3.5.1', 'learndash_quiz_complete_essay_graded' );
					}
					...
					if ( true === (bool) apply_filters( 'learndash_quiz_complete_essay_graded', $check_graded, $user_id, $quiz ) ) {
						foreach ( $quiz['graded'] as $question_id => $graded_set ) {
							if ( isset( $graded_set['post_id'] ) ) {
								$graded_post_status = get_post_status( absint( $graded_set['post_id'] ) );
								if ( 'not_graded' === $graded_post_status ) {
									$pass = false;
									break;
								}
							}
...
function learndash_next_lesson_quiz( $url = true, $user_id = null, $lesson_id = null, $exclude = array() ) {
	...
	if ( empty( $return ) ) {
		if ( ! learndash_can_complete_step( $user_id, $lesson_id ) ) {
 ...
 *
 * This is mostly used before auto-completing a prent step like a quiz
 * parent lesson.
 *
 * @since 3.2.3
 *
 ...
 * @param int $course_id Course ID.
 * @return boolean True if can complete.
 */
function learndash_can_complete_step( $user_id = 0, $step_id = 0, $course_id = 0 ) {
	...
	if ( ( ! empty( $user_id ) ) && ( ! empty( $step_id ) ) && ( ! empty( $course_id ) ) ) {
		if ( in_array( get_post_type( $step_id ), array( learndash_get_post_type_slug( 'lesson' ), learndash_get_post_type_slug( 'topic' ) ), true ) ) {
			...
			if ( ! learndash_user_is_course_children_progress_complete( $user_id, $step_id, $course_id ) ) {
				return false;
			}
		}
...
function learndash_show_mark_incomplete( $post, $atts = array() ) {
	...
	$form_fields = '<input type="hidden" value="' . $post->ID . '" name="post" />
				<input type="hidden" value="' . learndash_get_course_id( $post->ID ) . '" name="course_id" />
				<input type="hidden" value="' . wp_create_nonce( 'sfwd_mark_incomplete_' . get_current_user_id() . '_' . $post->ID ) . '" name="sfwd_mark_incomplete" />
 ...
 */
function learndash_mark_incomplete_process( $post = null ) {
	// This is wrong. This function hooks into the 'wp' action. That action doesn't pass a post object or post_id.
	// The $post object set here is not even used. We only need the _POST[post] (post_id) variable from the form.
	if ( empty( $post ) ) {
		global $post;
	}
