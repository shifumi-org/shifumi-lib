...
<?php
	...
	if ( ! empty( $lesson_progression_enabled ) ) :
		...
		if ( ! empty( $user_id ) ) {
			...
			if ( ( ! learndash_is_sample( $quiz_post ) ) && ( ( learndash_get_setting( $quiz_post->ID, 'visible_after' ) ) || ( learndash_get_setting( $quiz_post->ID, 'visible_after_specific_date' ) ) ) ) {
				$show_content         = false;
				$last_incomplete_step = false;
			} elseif ( learndash_is_sample( $quiz_post ) && ! empty( $user_id ) ) {
				$show_content = true;
			} elseif ( learndash_is_sample( $quiz_post ) && empty( $user_id ) ) {
				$show_content = false;
			} else {
