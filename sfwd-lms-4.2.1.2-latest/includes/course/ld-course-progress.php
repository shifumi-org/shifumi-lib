...
function learndash_quiz_redirect() {
	...
	if ( ! empty( $_GET['quiz_redirect'] ) && ! empty( $_GET['quiz_id'] ) && ! empty( $_GET['quiz_type'] ) && ! empty( $_GET['course_id'] ) && 'global' == $_GET['quiz_type'] ) {
		...
		if ( ! empty( $_GET['quiz_redirect'] ) && ! empty( $_GET['quiz_id'] ) && ! empty( $_GET['quiz_type'] ) && ! empty( $_GET['lesson_id'] ) && 'lesson' == $_GET['quiz_type'] ) {
			...
			if ( ( 1 === $next_step ) && ( ! empty( $user_id ) ) && ( ! empty( $course_id ) ) && ( ! empty( $lesson_id ) ) ) {
				if ( get_post_type( $lesson_id ) === learndash_get_post_type_slug( 'topic' ) ) {
					$parent_lesson_id = (int) learndash_course_get_single_parent_step( $course_id, $lesson_id, learndash_get_post_type_slug( 'lesson' ) );
				...
				if ( ! empty( $next_incomplete_step_id ) ) {
					// get the parent lesson of the next available step.
					$parent_step_lesson_id = (int) learndash_course_get_single_parent_step( $course_id, $next_incomplete_step_id, learndash_get_post_type_slug( 'lesson' ) );
					// If the parent lesson is not the same as the current lesson then redirect to the parent lesson.
					if ( ( ! empty( $parent_step_lesson_id ) ) && ( $parent_step_lesson_id !== $lesson_id ) ) {
						$next_incomplete_step_id = $parent_step_lesson_id;
					}
