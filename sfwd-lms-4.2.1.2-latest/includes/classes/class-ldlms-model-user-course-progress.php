 ...
 * @param integer $step_id          Course Step ID.
 *
 * @return integer|false The next incomplete step id or false if none found.
 */
function learndash_user_progress_get_next_incomplete_step( $user_id = 0, $course_id = 0, $step_id = 0 ) {
	...
	foreach ( $course_progress_steps as $progress_step_key => $progress_status ) {
		if ( false === $found_key ) {
			...
			//$previous_item = learndash_get_previous( $progress_step_post_id );

			$progress_step_post_id = absint( $progress_step_post_id );

			$progress_step_post_id = learndash_user_progress_get_parent_incomplete_step( $user_id, $course_id, $progress_step_post_id );
