...
function _TCAPI_GetState(key) {
	var result;
	try {
		result = tincan.getState(key);
	} catch (ex) {
		WriteToDebug("In _TCAPI_SetStateSafe - caught exception from setState: " + ex.message);
	}
	return result;
...
function _TCAPI_LMSGetState(key) {
	var result;
	try {
	...
	if( typeof result !== "undefined" && typeof result.state !== "undefined")
		return result.state.contents;
...
function TCAPI_LMSGetLastError(_self){
	var returnValue = 0;

	return returnValue;
}
function TCAPI_LMSGetErrorString(CMIErrorCode) {
	var returnValue = "";
...
function TCAPI_GetLastError() {
  ...
  if (intTCAPIError === TCAPI_NO_ERROR) {
    ...
    return NO_ERROR;
  	...
  	if( typeof intTCAPIError == 'undefined'){
		intTCAPIError = 0;
	}
...
function LMSStandardAPI(strStandard) {
  ...
  this.LMSInitialize = TCAPI_Initialize;
  this.Finish = TCAPI_Finish;
  this.LMSFinish = TCAPI_Finish;
  this.Terminate = TCAPI_Finish;
  this.CommitData = TCAPI_CommitData;
  this.Commit = CommitData;
  this.LMSCommit = CommitData;
  this.GetValue = _TCAPI_GetState;
  this.LMSGetValue = _TCAPI_LMSGetState;
  this.SetValue = _TCAPI_SetStateSafe;
  this.LMSSetValue = _TCAPI_SetStateSafe;
  ...
  this.LMSGetLastError = TCAPI_GetLastError;
  this.GetLastErrorDesc = TCAPI_GetLastErrorDesc;
  this.LMSGetErrorString = TCAPI_GetLastErrorDesc;
  this.GetErrorString = TCAPI_GetLastErrorDesc;
...
function LMSStandardAPIEval(strStandard) {
  ...
  eval("this.GetValue = " + strStandard + "_GetState");
...
strStandAlone = GetQueryStringValue("sv", window.location.search);

//initializeCommunication = Start;
if( strStandAlone == '2004' ) {
	var API_1484_11 = new LMSStandardAPI('TCAPI');
} else {
	var API = new LMSStandardAPI('TCAPI');
}
