                            ...
                            if ($do_not_update_user){
                                ...
                                if( !empty( $_POST['is_subscription'] ) && 1 == $_POST['is_subscription'] ){
                                    ...
                                    if( is_wp_error( $stripe_plan_data ) ){
                                        ...
                                        $sch_request_string = 'customer=' . $customer_id . '&start_date='.$arm_subscription_start_date.'&phases[0][items][0][price]='.$charge_details['plan'].$phases_metadata_str;
                ...
                if(isset($all_payment_gateways['stripe']) && !empty($all_payment_gateways['stripe'])){
                    ...
                    if( isset( $response->type) && 'invoice.payment_succeeded' == $response->type ){
                            ...
                            if( is_wp_error($wp_post_data) ){
                                ...
                                if(isset( $subscription_data->status) && ('active' == $subscription_data->status || 'paid' == $subscription_data->status || 'trialing' == $subscription_data->status)){
                                    ...
                                    if(is_wp_error($wp_post_data)){
                                        ...
                                        $arm_subscription_id_field_name = "id";
                                        $arm_token_field_name = "";
                                        $arm_transaction_field_name = "";
                        ...
                        if( !empty( $schedule_id ) ){
                            $retrieve_payment_log = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM `' . $ARMember->tbl_arm_payment_log . '` WHERE arm_token = %s AND arm_transaction_id = %s AND arm_transaction_payment_type = %s', $customer_id, $schedule_id, 'subscription' ) );
