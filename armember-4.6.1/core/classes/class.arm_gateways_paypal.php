            ...
            if ($payment_gateway == 'paypal') {
                ...
                if(!empty($plan_id) && !empty($entry_id))
                {
                    ...
                    if(!empty($arm_return_data['arm_coupon_data'])){
                        ...
                        $arm_coupon_amount = !empty($arm_coupon_data['coupon_amt']) ? $arm_coupon_data['coupon_amt'] : 0;
                        $arm_coupon_amount = str_replace(',', '', $arm_coupon_amount);

                        $coupon_code = $arm_coupon_data['arm_coupon_code'];
                        $arm_coupon_discount = $arm_coupon_data['discount'];

                        $arm_plan_amount = (float)$arm_return_data['arm_plan_obj']->amount;
                        $amount = $arm_plan_amount;
                        
                        if(!empty($arm_coupon_on_each_subscriptions)){
                            if($arm_is_trial && ($arm_final_trial_amount > 0)){
                                $amount = $arm_final_trial_amount;
                            } else if($arm_is_trial && $arm_final_trial_amount == 0) {
                                if($arm_coupon_data['discount_type'] == 'percentage'){
                                    $amount = $amount - ($amount * ($arm_coupon_discount/100));
                                }else{
                                    $amount = $amount - $arm_coupon_discount;
                                }
                            } else {
                                $amount = $arm_plan_amount - $arm_coupon_amount;
                            }
                        } else {
                            if(!$arm_is_trial && $payment_mode == 'auto_debit_subscription'){
                                $arm_is_trial = 1;
                                $arm_final_trial_amount = ($arm_plan_amount - $arm_coupon_amount);
                                $trial_period = $recur_period;
                                $trial_interval = $recur_interval;
                            } else {
                                if($arm_is_trial && ($arm_final_trial_amount > 0)){
                                    $amount = $arm_final_trial_amount; 
                                } else if(!$arm_is_trial) {
                                    $amount = $arm_plan_amount - $arm_coupon_amount;
                                }
                            }
                        }
                        ...
                        if(!empty($arm_return_data['arm_tax_data'])){
                            if($arm_final_trial_amount > 0 && empty($arm_return_data['arm_recurring_data']['trial'])){
                                /**
                                 * It needs to calculate manually because when plan actually have no trial and coupon only applied for first recurring then we consider first payment as trial. 
                                 * So that's why we have to calculate payment with trial amount again.
                                */
                                $arm_tax_per = $arm_return_data['arm_tax_data']['tax_percentage'];
                                $arm_tax_val = $arm_final_trial_amount * ($arm_tax_per / 100);
                                $arm_final_trial_amount = $arm_final_trial_amount + $arm_tax_val;
                            }

                            if(($amount == 0 && $arm_is_trial && $arm_final_trial_amount == 0 && empty($arm_coupon_on_each_subscriptions)) || ($amount > 0 && $arm_is_trial && $arm_final_trial_amount > 0) || ($amount > 0 && $arm_is_trial && $arm_final_trial_amount == 0)){
                                $amount = $arm_return_data['arm_tax_data']['tax_final_amount'];
                            }
                            else if($amount > 0 && !$arm_is_trial){
                                $arm_tax_val = $arm_return_data['arm_tax_data']['tax_amount'];
                                $amount = $amount + $arm_tax_val;
                            }
                        }

                        if(!empty($arm_coupon_data['discount_type'])){
                    ...
                    if ($plan->is_recurring() && $payment_mode == 'auto_debit_subscription') {
                        ...
                        if(($arm_is_trial && $plan_action == 'new_subscription') || $remained_days > 0) {
                    ...
                    if(!empty($all_payment_gateways['paypal'])){
					    ...
					    //No Need this txn_type because subscr_payment sending same details with transaction ID
                                    ...
                                    case 'recurring_payment_profile_cancel':
                                        ...
                                        if(empty($user_id) || ($planData['arm_paypal']['arm_subscr_id'] != $arm_token) )
                    					{
                    						return;
                    					}
