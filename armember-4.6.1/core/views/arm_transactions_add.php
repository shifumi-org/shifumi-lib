<?php 
global $wpdb, $ARMember, $arm_slugs, $arm_global_settings, $arm_members_class, $arm_manage_coupons, $arm_payment_gateways, $arm_subscription_plans,$arm_pay_per_post_feature;
			...
			<div class="arm_admin_form_content">
				...
				<tr class="form-field form-required"><th style="padding: 10px;"><span style="color: red;"><?php _e('Important Note', 'ARMember');?>:</span></th><td><span style="color: red;"><?php _e('The only purpose of this form is to add missed payment records of users for keeping track of their all payments. So, it doesn\'t mean that, when you add paymnet from here for any plan, it will renew next payment cycle or any plan will be assigned to user.', 'ARMember'); ?>
					...
					<tr class="form-field arm_auto_paid_post_field arm_transaction_paid_post_plan_wrapper hidden_section">
						...
						<td>
							<input id="arm_paid_post_plan_auto_selection" type="text" name="arm_paid_post_plan_ids" value="" placeholder="<?php _e('Search by paid post plan...', 'ARMember');?>" data-msg-required="<?php _e('Please select paid post plan.', 'ARMember');?>" required>
