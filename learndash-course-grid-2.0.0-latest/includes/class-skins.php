...
class Skins
{
    ...
    public function enqueue_skin_assets()
    {
        ...
        if ( $post && $post->post_type === 'page' && preg_match( '/\[ld_.*?_list/', $post->post_content ) && ! preg_match( '/\[ld_.*?_list.*?course_grid=(?:"|\')*false(?:"|\')*/', $post->post_content ) ) {
            $skin  = 'legacy-v1';
    ...
    public function enqueue_editor_skin_assets()
    {
        ...
        foreach ( $cards as $id => $card ) {
            ...
            if ( $script_file ) {
                wp_enqueue_script( 'learndash-course-grid-card-' . $id, $script_file, $script_keys, LEARNDASH_COURSE_GRID_VERSION, true );
            }
        }
