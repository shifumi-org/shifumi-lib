...
if (
	...
	class Learndash_Admin_Export_File_Handler extends Learndash_Admin_Import_Export_File_Handler {
		...
		public function init_zip() {
			...
			$this->zip_dir_path = $upload_dir['basedir'] . $zip_dir;
			$this->zip_dir_url  = $upload_dir['baseurl'] . $zip_dir;
			...
			if ( ! empty( $zip_files ) ) {
				...
				$this->zip_archive_url  = $this->zip_dir_url . DIRECTORY_SEPARATOR . basename( $this->zip_archive_path );
			} else {
				...
				$this->zip_archive_url  = $this->zip_dir_url . DIRECTORY_SEPARATOR . $zip_file_name;
			}
		}
