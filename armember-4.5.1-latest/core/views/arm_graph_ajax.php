...
if ($plan_type == "members") {
    ...
    if ($type == "yearly") 
    {
        ...
        if(!empty($table_data_arr)) {
            ...
            foreach ($table_data_arr as $key => $table_data) {
                ...
                $userlogin = (!empty($table_data['user_login'])) ? $table_data['user_login'] : $arm_default_user_details_text;
                $useremail = (!empty($table_data['user_login'])) ? "<a class='arm_openpreview_popup' href='javascript:void(0)' data-id='".$table_data['ID']."'>".$table_data['user_email']."</a>" : $arm_default_user_details_text;
    ...
    $table_data_arr = array();
    $table_data_cnt = 0;
    $table_data_cnt_arr = $wpdb->get_results("SELECT arm_log_id, arm_created_date, arm_payment_gateway,arm_coupon_code,arm_coupon_discount,arm_coupon_discount_type, arm_user_id FROM ({$ctgquery}) AS arm_payment_history_log_data", 'ARRAY_A');    
    ...
    <?php if(isset($_REQUEST['action']) && $_REQUEST['action']=="armupdatecharts"){?>
        ...
        ?>
                <div class="armchart_plan_item">
                    ...
                    <p class="armchart_plan_item_desc"><?php echo __("Coupon Code Used:", "ARMember");?> <?php echo count($arm_charts_plan_row['coupon_users']); ?></p>
                </div>
        <?php 
                }
            }else {
        ...
        <div class="armchart_view_section">
    <?php }?>
        <div id="daily_coupon_report" style="padding:15px;<?php echo ($type == 'daily') ? 'display:block;' : 'display:none'; ?>">
            <?php if(!isset($_REQUEST['action'])) { ?>
            <div class="armchart_display_title">
                <label class="armcharttitle">
                    <?php echo date(get_option('date_format'), strtotime($new_day . '-' . $new_day_month . '-' . $new_day_year)); ?>
            ...
            <?php }?>
            <div id="chart1_coupon_report" style="width:100%;<?php echo ($graph_type == 'countries') ? 'height:400px;' : 'height:300px;';?>" ></div>
        ...
        <div id="monthly_coupon_report" style="padding:15px; <?php echo ($type == 'monthly') ? 'display:block;' : 'display:none'; ?>">
            <?php if(!isset($_REQUEST['action'])){?>
            <?php $monthName = date("F", mktime(0, 0, 0, (int)$new_month, 10)); ?>
            <div class="armchart_display_title">
                <label class="armcharttitle"><?php echo $monthName . "-" . $new_month_year; ?></label>
            ...
            <?php }?>
            <div id="chart2_coupon_report" style="width:100%;<?php echo ($graph_type == 'countries') ? 'height:400px;' : 'height:300px;';?>" ></div>
            <input type="hidden" value="<?php echo $new_month; ?>" name="current_month" id="current_month" />
            <input type="hidden" value="<?php echo $new_month_year; ?>" name="current_month_year" id="current_month_year" />
        </div>

        <div id="yearly_coupon_report" style="padding:15px; <?php echo ($type == 'yearly') ? 'display:block;' : 'display:none'; ?>">
            <div class="arlinks link_align"></div>
            <?php if(!isset($_REQUEST['action'])){?>
            <div class="armchart_display_title">
                <label class="armcharttitle"><?php echo $new_year; ?></label>
            ...
            <?php }?>
            <div id="chart3_coupon_report" style="width:100%;<?php echo ($graph_type == 'countries') ? 'height:400px;' : 'height:300px;';?>" ></div>
            <input type="hidden" value="<?php echo $new_year; ?>" name="current_year" id="current_year" />
        </div>
    <?php if(isset($_REQUEST['action']) && $_REQUEST['action']=="armupdatecharts"){?>
    </div>
