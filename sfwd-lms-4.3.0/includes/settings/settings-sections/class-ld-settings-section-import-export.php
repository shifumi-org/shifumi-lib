	...
	class LearnDash_Settings_Section_Import_Export extends LearnDash_Settings_Section {
			...
			?>
				...
				>
					<p>
						...
						?>
						<a download href="<?php echo esc_url( $this->export_file_handler->get_zip_archive_url() ); ?>">
				...
				<?php if ( isset( $_GET['LD_DEBUG'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
					<div class="notice notice-info is-dismissible">
						<?php
							$export_log_link = $this->export_file_handler->get_log_file_link();
							$import_log_link = $this->import_file_handler->get_log_file_link();
						?>

						<?php if ( ! empty( $export_log_link ) ) : ?>
							<p>
								<a download href="<?php echo esc_url( $export_log_link ); ?>">
									<?php esc_html_e( 'Download last export log', 'learndash' ); ?>
								</a>
							</p>
						<?php endif; ?>

						<?php if ( ! empty( $import_log_link ) ) : ?>
							<p>
								<a download href="<?php echo esc_url( $import_log_link ); ?>">
		...
		protected function get_settings(): array {
			...
			foreach ( LDLMS_Post_Types::get_post_types() as $post_type ) {
				...
				if ( isset( Learndash_Admin_Export_Post_Type_Settings::POST_TYPE_SETTING_PAGE[ $post_type_key ] ) ) {
					$setting['items'][] = array(
		...
		public function show_meta_box(): void {
			...
			<div id="learndash-import-export" class="sfwd_options">
				...
				>
					...
					<div>
						<div>
							<input autocomplete="off" type="file" id="learndash-import-file" accept="application/zip">
						</div>
