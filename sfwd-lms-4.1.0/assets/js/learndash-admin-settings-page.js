...
jQuery(function($) {
	...
	$('.view-learndash-assignment').on('click', function(e) {
		...
		const is_image = $(this).data('is-image');

		const label = is_image
			? '<img src=\"' + link + '\" />'
			: learndash_admin_settings_data.labels.download;

		const content = '<a download href=\"' + link + '\">' + label + '</a>';
				...
				if (response.success) {
					$('#learndash-admin-table-modal')
						...
						.dialog({
							...
							draggable: false,
							resizable: false,
							maxHeight: response.data.height,
							width: 600,
