...
if ( ( class_exists( 'Learndash_Admin_Posts_Listing' ) ) && ( ! class_exists( 'Learndash_Admin_Essays_Listing' ) ) ) {
	...
	class Learndash_Admin_Essays_Listing extends Learndash_Admin_Posts_Listing {
		...
		public function add_inline_actions( array $row_actions, WP_Post $post ): array {
			...
			if ( ! empty( $file_url ) ) {
				$row_actions['download_essay'] = sprintf(
			...
			$view_label = __( 'View' ); // phpcs:ignore
			$view_url   = admin_url(
				sprintf(
		...
		public static function load_modal_content(): void {
			...
			if ( empty( $file_url ) ) {
				...
				$file_is_image = in_array(
					pathinfo( $file_url, PATHINFO_EXTENSION ),
					array( 'jpg', 'jpeg', 'png', 'gif' ),
					true
				);

				$label = $file_is_image
					? sprintf( '<img src="%s" />', $file_url )
					: esc_html__( 'Download', 'learndash' );

				$content = sprintf(
					'<a download href="%s">%s</a>',
					esc_url( $file_url ),
					$label
				);

				if ( ! $file_is_image ) {
					$content = '<div class="learndash-modal-download-link-wrapper">' . $content . '</div>';
				}
			}
			...
			wp_send_json_success(
				[
					...
					'content' => $content,
					'height'  => empty( $file_url )
						? 500
						: ( $file_is_image ? 500 : 150 ),
				]
			);
