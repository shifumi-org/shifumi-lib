...
if ( ! class_exists( 'LearnDash_Stripe_Connect_Checkout_Integration' ) ) {
	...
	class LearnDash_Stripe_Connect_Checkout_Integration {
		...
		protected function update_course_access( int $course_id, int $user_id, bool $remove = false ): void {
			...
			if ( learndash_get_post_type_slug( 'course' ) === $post_type_slug ) {
				ld_update_course_access( $user_id, $course_id, $remove );
			} elseif ( learndash_get_post_type_slug( 'group' ) === $post_type_slug ) {
				ld_update_group_access( $user_id, $course_id, $remove );
			}
		}
		 ...
		 */
		public function record_transaction( $session, int $course_id, int $user_id, string $user_email ) {
			...
			$transaction_id = wp_insert_post(
				array(
					'post_title'  => $transaction_data['stripe_name'] . __( ' Purchased By ', 'learndash' ) . $user_email,
