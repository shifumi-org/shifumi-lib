            ...
            if ($payment_gateway == 'paypal') {
                ...
                if(!empty($plan_id) && !empty($entry_id))
                {
                    ...
                    if(!empty($arm_return_data['arm_coupon_data'])){
                        ...
                        $arm_plan_amount = $arm_return_data['arm_plan_obj']->amount;
                        ...
                        if(!empty($arm_coupon_on_each_subscriptions)){
                            ...
                            if(!$arm_is_trial && $payment_mode == 'auto_debit_subscription'){
                                ...
                                if($arm_is_trial){
                                    ...
                                    $amount = $arm_plan_amount - $arm_coupon_data['coupon_amt'];
                                }
                            }
                        }
                            ...
                            if(!empty($txn_id) || !empty($arm_token)){
                                switch($txn_type){
                                    ...
                                    case 'recurring_payment_profile_cancel':
                                        ...
                                        $paypalLog['txn_id'] = $arm_transaction_id_arr['arm_transaction_id'];
                                        ...
                                        if($paly_cycle_data['rec_time'] != 'infinite' || $plan->options['cancel_plan_action'] != "on_expire")
                                        {
                                            if (!empty($planData) && (!empty($planData['arm_paypal']['arm_subscr_id']) || !empty($arm_token)) && empty($planData['arm_cencelled_plan']) )
                                            {
                                                ...
                                                do_action('arm_cancel_subscription_payment_log_entry', $user_id, $entry_plan, 'paypal', $arm_token, $arm_transaction_id_arr['arm_transaction_id'], '', '', $arm_cancel_amount);

                                                do_action('arm_after_recurring_payment_cancelled_outside', $user_id, $entry_plan, 'paypal');
                                            }
                                        }
                                        else if((!empty($planData['arm_paypal']['arm_subscr_id']) || !empty($arm_token)) && is_array($planData) && $paly_cycle_data['rec_time']=='infinite' && $plan->options['cancel_plan_action']== "on_expire") 
                                        {
